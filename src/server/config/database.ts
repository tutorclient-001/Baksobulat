...
// In-memory / fallback store for development when DATABASE_URL is not configured
...
export async function initDatabase(): Promise<{ isPostgres: boolean; message: string }> {
  if (config.DATABASE_URL && config.DATABASE_URL.trim() !== '') {
    try {
      ...
      return { isPostgres: true, message: 'Connected to Neon PostgreSQL' };
    } catch (err: any) {
      console.warn('Could not connect to PostgreSQL URL. Falling back to in-memory active store:', err.message);
      isConnectedToPostgres = false;
      pool = null;
      return { isPostgres: false, message: `PostgreSQL connection error: ${err.message}` };
    }
  } else {
    console.log('DATABASE_URL not set. Running in resilient local storage mode.');
    isConnectedToPostgres = false;
    return { isPostgres: false, message: 'Running in Local Storage Mode (Ready for Neon PostgreSQL)' };
  }
}
...
export async function query<T extends QueryResultRow = any>(
  ...
  if (pool && isConnectedToPostgres) {
    return pool.query<T>(text, params);
  }
  throw new Error('Database pool not connected.');
}

export async function checkDbHealth(): Promise<{ healthy: boolean; type: string; latencyMs?: number; error?: string }> {
  if (pool && isConnectedToPostgres) {
    const start = Date.now();
    try {
      await pool.query('SELECT 1');
      return { healthy: true, type: 'Neon / PostgreSQL', latencyMs: Date.now() - start };
    } catch (e: any) {
      return { healthy: false, type: 'PostgreSQL', error: e.message };
    }
  }
  return { healthy: true, type: 'Local Storage (Interactive Mode)' };
}
